...
class Role(models.Model):
    ...
    can_create_products = models.BooleanField(
        default=False, verbose_name="Criar produtos"
    )
    can_edit_products = models.BooleanField(
        default=False, verbose_name="Editar produtos"
    )
    can_delete_products = models.BooleanField(
        default=False, verbose_name="Excluir produtos"
    )
    can_view_products = models.BooleanField(default=True, verbose_name="Ver produtos")

    can_create_suppliers = models.BooleanField(
        default=False, verbose_name="Criar fornecedores"
    )
    can_edit_suppliers = models.BooleanField(
        default=False, verbose_name="Editar fornecedores"
    )
    can_delete_suppliers = models.BooleanField(
        default=False, verbose_name="Excluir fornecedores"
    )
    can_view_suppliers = models.BooleanField(
        default=True, verbose_name="Ver fornecedores"
    )

    can_create_sales = models.BooleanField(
        default=False, verbose_name="Criar movimentações"
    )
    can_edit_sales = models.BooleanField(
        default=False, verbose_name="Editar movimentações"
    )
    can_delete_sales = models.BooleanField(
        default=False, verbose_name="Excluir movimentações"
    )
    can_view_sales = models.BooleanField(default=True, verbose_name="Ver movimentações")

    can_create_reports = models.BooleanField(
        default=False, verbose_name="Criar relatórios"
    )
    can_view_reports = models.BooleanField(default=True, verbose_name="Ver relatórios")

    can_manage_users = models.BooleanField(
        default=False, verbose_name="Gerenciar usuários"
    )
    can_view_audit_logs = models.BooleanField(
        default=False, verbose_name="Ver logs de auditoria"
    )
    ...
    def create_for_company(cls, company, role_name="admin"):
        ...
        role, created = cls.objects.get_or_create(
            name=role_name, company=company, defaults=permissions
        )
        return role
...
class AuditLog(models.Model):
    ...
    )  # Backup caso user seja deletado
    ...
    object_id = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="ID do objeto"
    )
    content_object = GenericForeignKey("content_type", "object_id")
    object_repr = models.CharField(
        max_length=255, blank=True, verbose_name="Representação do objeto"
    )

    # Detalhes da mudança
    changes = models.JSONField(default=dict, blank=True, verbose_name="Mudanças")
    old_values = models.JSONField(
        default=dict, blank=True, verbose_name="Valores anteriores"
    )
    new_values = models.JSONField(
        default=dict, blank=True, verbose_name="Novos valores"
    )
...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    role = models.CharField(
        max_length=20, choices=ROLE_CHOICES, default="operator", verbose_name="Papel"
    )
    ...
    company = models.CharField(
        max_length=150, blank=True, null=True, verbose_name="Nome da Empresa (legado)"
    )
    ...
    def can_delete(self):
        ...
        return (
            self.role in ["instock_admin", "company_admin", "manager"]
...
class AccessRequest(models.Model):
    ...
    company_name = models.CharField(
        max_length=150, verbose_name="Nome da empresa", blank=True, null=True
    )
