...
class SupplierModelTests(TestCase):
    ...
    def test_supplier_creation(self):
        ...
        self.assertEqual(self.supplier.address, "Rua Principal, 123 - São Paulo, SP")
    ...
    def test_supplier_cnpj_uniqueness(self):
        ...
        with self.assertRaises(Exception):
            Supplier.objects.create(name="Another Supplier", cnpj="12345678000190")

    def test_supplier_ordering(self):
        """Testa se os fornecedores são ordenados por nome"""
        Supplier.objects.create(name="ABC Supplier", cnpj="98765432000190")
        Supplier.objects.create(name="XYZ Supplier", cnpj="11111111000190")
        suppliers = list(Supplier.objects.all().values_list("name", flat=True))
        self.assertEqual(suppliers, ["ABC Supplier", "Tech Supplier", "XYZ Supplier"])

    def test_supplier_optional_fields(self):
        """Testa se campos opcionais podem ser deixados em branco"""
        supplier = Supplier.objects.create(
            name="Minimal Supplier", cnpj="55555555000190"
        )
